    ...
    <div className="min-h-screen bg-dark p-6">
      <div className="max-w-6xl mx-auto">
        <header className="text-center mb-8">
          <h1 className="text-4xl font-bold text-primary mb-2">Professional Medical Certificate Generator</h1>
          <p className="text-gray-400 max-w-2xl mx-auto">Generate professional medical certificates for your patients with instant PDF download.</p>
        </header>

        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          {/* Form Section */}
          <div className="glass-card p-6 rounded-lg shadow-md border border-white/10">
            <h2 className="text-xl font-semibold text-primary mb-4 pb-2 border-b border-white/10">Certificate Information</h2>

            {validationError && (
              <div className="mb-4 p-3 bg-red-500/10 text-red-400 rounded-md border border-red-500/50">
                {validationError}
              </div>
            )}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                <input
                  ...
                  className="w-full p-2 border border-white/20 rounded bg-surface text-white focus:ring-2 focus:ring-primary focus:border-transparent focus:outline-none"
              ...
              <div>
                <label className="block text-sm font-medium text-gray-300 mb-1">Patient Age</label>
                <input
                  type="number"
                  placeholder="Age"
                  className="w-full p-2 border border-white/20 rounded bg-surface text-white focus:ring-2 focus:ring-primary focus:border-transparent focus:outline-none"
              ...
              <div>
                <label className="block text-sm font-medium text-gray-300 mb-1">Patient Gender</label>
                <select
                  className="w-full p-2 border border-white/20 rounded bg-surface text-white focus:ring-2 focus:ring-primary focus:border-transparent focus:outline-none"
                  value={formData.patientGender}
                  onChange={(e) => setFormData({ ...formData, patientGender: e.target.value })}
                >
                  <option value="" className="bg-dark">Select Gender</option>
              ...
              <div>
                <label className="block text-sm font-medium text-gray-300 mb-1">Doctor Specialty</label>
                <input
                  type="text"
              ...
              <div className="md:col-span-2">
                ...
                <textarea
                  ...
                  className="w-full p-2 border border-white/20 rounded bg-surface text-white focus:ring-2 focus:ring-primary focus:border-transparent focus:outline-none h-24"
            ...
            <div className="flex gap-4 mt-6">
              <button
                ...
                className="flex-1 bg-primary hover:bg-primary/80 text-white font-bold py-3 px-4 rounded transition ease-in-out flex items-center justify-center"
                disabled={loading}
              >
                {loading ? (
              ...
              <button
                onClick={resetForm}
                className="bg-white/10 hover:bg-white/20 text-gray-300 font-medium py-3 px-4 rounded transition ease-in-out"
              >
                Reset
              </button>
            </div>
          ...
          {/* Preview Section */}
          <div>
            <div className="glass-card p-6 rounded-lg shadow-md mb-6 border border-white/10">
              <div className="flex justify-between items-center mb-4">
                <h2 className="text-xl font-semibold text-primary">Certificate Preview</h2>
                <button
                  onClick={generatePDF}
                  className="bg-primary hover:bg-primary/80 text-white font-bold py-2 px-4 rounded transition ease-in-out flex items-center"
              ...
              <div className="border border-white/20 rounded-lg p-4 bg-white min-h-80">
                <div className="whitespace-pre-line font-serif text-gray-900">{certificateText}</div>
              </div>
            </div>
            ...
            {certificates.length > 0 && (
              <div className="glass-card p-6 rounded-lg shadow-md border border-white/10">
                <h2 className="text-xl font-semibold text-primary mb-4">Recent Certificates</h2>
                <div className="overflow-y-auto max-h-40 custom-scrollbar">
                  {certificates.map(cert => (
                    <div key={cert.id} className="p-3 border-b border-white/10 hover:bg-white/5 cursor-pointer" onClick={() => setCertificateText(cert.content)}>
                      <p className="font-medium text-white">{cert.patientName}</p>
